...
def inference_pathgnn(model, x, paths, path_types):
    model.eval()
    with torch.no_grad():
        logits = model(x, paths, path_types)
    return logits

def evaluate_pathgnn(args, model, x, paths, path_types, criterion, device, mask, original_x, save_path=None, imputed_data_save_path=None):
    model.eval()
    total_loss = 0
    ...
    with torch.no_grad():
        ...
        # Compute the logits
        logits = model(x, paths, path_types, save_path)
    ...
    if imputed_data_save_path != None:
        np.save(imputed_data_save_path, logits.detach().cpu().numpy())
        
    loss = F.mse_loss(all_masked_logits, all_masked_original_x)
    k = sum(p.numel() for p in model.parameters() if p.requires_grad)
    n = all_masked_logits.size(0) 
    bic = k * torch.log(torch.tensor(n).float()) - 2 * (-loss * n)

    return l1_distance, cosine_sim, rmse, bic

def inference_with_batch(args, model, sampler, data, device):
    model.eval()
    all_logits = []
    with torch.no_grad():
        for batch_size, n_id, adjs in sampler:
            ...
            all_logits.append(target_logits)  # Collect the output for each batch
